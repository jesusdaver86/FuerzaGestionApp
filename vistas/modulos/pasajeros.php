...
<div id="modalAgregarPasajero" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form role="form" method="post" id="agregarPasajeroForm">
                <!--=====================================
                CABEZA DEL MODAL
                ======================================-->
            ...
            <div id="mensajeAgregarPasajero" class="alert" style="display:none;"></div>
        </div>
    </div>
</div>
...
<script>
$(document).ready(function() {
    $("#agregarPasajeroForm").submit(function(event) {
        ...
        $.ajax({
            type: "POST",
            url: "ajax/pasajeros.ajax.php", // Correct URL
            data: formData + "&accion=crearPasajero", // Add an action parameter
            dataType: "json", // Expect a JSON response
            success: function(response) {
                if (response.success) {
                    $("#mensajeAgregarPasajero").removeClass("alert-danger").addClass("alert-success").text(response.message).show();
                    $("#agregarPasajeroForm")[0].reset(); // Reset the form
                    $(".tablas").DataTable().ajax.reload(); // Reload the DataTable
                    setTimeout(function() {
                        $("#modalAgregarPasajero").modal('hide'); // Hide the modal
                        $("#mensajeAgregarPasajero").hide(); // Hide the message
                    }, 2000); // Hide the message after 2 seconds
                } else {
                    $("#mensajeAgregarPasajero").removeClass("alert-success").addClass("alert-danger").text(response.message).show();
                }
            },
            error: function(xhr, status, error) {
                $("#mensajeAgregarPasajero").removeClass("alert-success").addClass("alert-danger").text("Error: " + error).show();
            }
        });
    });
});
...
$(document).ready(function() {
    $('.tablas').DataTable({
        "language": {
            "url": "../node_modules/datatable/DataTables/js/es-ES2.json"
        }
    });
});
</script>
